...
def generate_fs(x, y, overloads):
    common_stuff = """
        template<int N> struct index_constant {};
    ...
    if y == 0:
        return enable_if(x) + common_stuff + "\n".join([
            "template<int N> auto f(index_constant<N>, priority_tag<%d>) -> enable_if_t<N==%d, A<%d>>;" % (i, i, i)
            for i in range(overloads)
        ])
    elif y == 1:
        return enable_if(x) + common_stuff + "\n".join([
            "template<int N, class = enable_if_t<N==%d>> auto f(index_constant<N>, priority_tag<%d>) -> A<%d>;" % (i, i, i)
            for i in range(overloads)
        ])
    elif y == 2:
        return enable_if(x) + common_stuff + "\n".join([
            "template<int N, enable_if_t<N==%d, int> = 0> auto f(index_constant<N>, priority_tag<%d>) -> A<%d>;" % (i, i, i)
            for i in range(overloads)
        ])
    elif y == 3:
        return common_stuff + "\n".join([
            "template<int N> requires (N==%d) auto f(index_constant<N>, priority_tag<%d>) -> A<%d>;" % (i, i, i)
            for i in range(overloads)
        ])
...
def generate_file(x, y, overloads):
    return generate_fs(x, y, overloads) + """
        void test() {
    """ + "\n".join([
        "f(index_constant<%d>{}, priority_tag<%d>{});" % (i, overloads)
        for i in range(overloads)
    ]) + """
        }
    """
